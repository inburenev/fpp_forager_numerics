					...
					Result_data *result_data)
{
	...
	if (argument_index < argc) {
		if (strcmp(argv[argument_index], "exp_M") == 0
			...
			if (argument_index + 1 < argc) {
				argument_index++;
				simulation_parameters->M_parameters[0] = atof(argv[argument_index]);
			}
			}
	} else {
	...
	if (argument_index < argc) {
		if (strcmp(argv[argument_index], "traj_len") == 0) {
			if (argument_index + 1 < argc) {
				argument_index++;
				simulation_parameters->trajectory_length = atoi(argv[argument_index]);
			}
		}
	} else {
		fprintf(stderr, "Invalid number of steps.\n");
	...
	if (argument_index < argc) {
		if (strcmp(argv[argument_index], "imp_s") == 0) {
			if (argument_index + 2 < argc) {
				...
				argument_index++;
				simulation_parameters->quantity_of_interest = argv[argument_index][0];
			}
		}
	} else {
...
int main(int argc, char *argv[]) {
	...
	time_start = clock();
	int time_taken_s, time_left_s; /* elapsed time and remaining time in seconds */
	double it_per_sec;
